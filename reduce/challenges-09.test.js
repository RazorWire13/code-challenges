...
const countNumberOfElements = (input) => {
  // Solution code here...
  return input.reduce ((acc, curr, idx) => idx + 1)
};
...
const countNumberOfChildren = (input) => {
  // Solution code here...
  return input.reduce((acc, current) =>  acc + current.children ? acc + current.children.length : acc, 0);
};
...
const extractStat = (statName, input) => {
  ...
  return input.reduce((acc, curr) => {
      if (!acc && curr.stat.name === statName) {
          return acc = curr;
      }
...
const calculateAverage = (input) => {
  // Solution code here...
  return input.reduce((acc, current) => (acc + current)) / input.length;
