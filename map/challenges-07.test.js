...
const forLoopTwoToThe = (input) => {
  ...
  for (let i = 0; i < input.length; i++) {
    forPowerOfTwo.push(2 ** input[i]);
}
return forPowerOfTwo;
};
...
const forEachTwoToThe = (input) => {
  // Solution code here...
  const forEaPowerOfTwo = [];
  input.forEach( (integer) => {
      forEaPowerOfTwo.push(2**integer);
  })
  return forEaPowerOfTwo;
};
...
const mapTwoToThe = (input) => {
  // Solution code here...
  return input.map( input => 2 ** input)
};
...
const charCode = (input) => {
  // Solution code here...
  return input.map( element => element.charCodeAt(input))
};
