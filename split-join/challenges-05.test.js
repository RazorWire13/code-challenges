...
const listFoods = (recipe) => {
    ...
    ingredients.forEach((ingredients) => {
        const idNumbers = ingredients.indexOf(' ');
        const removeNumbers = ingredients.slice(idNumbers + 1);
        const idAmounts = removeNumbers.indexOf(' ');
        const removeAmounts = removeNumbers.slice(idAmounts + 1);
        console.log(removeAmounts);
        result.push(removeAmounts);
    })
...
const stepActions = (recipe) => {
    ...
    for (let i = 0; i < steps.length; i++) {
      const index = steps[i].indexOf(' ');
      console.log(steps[i].indexOf(' '))
      result.push(steps[i].slice(0, index));
      console.log(steps[i]);
    }
    return result;
